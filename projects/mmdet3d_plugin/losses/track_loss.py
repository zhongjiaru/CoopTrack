...
from projects.mmdet3d_plugin.cooptrack.dense_heads.track_head_plugin import Instances
...
class ClipMatcher(nn.Module):
    def __init__(
        ...
        self.pc_range = self.matcher.pc_range
    ...
    def loss_boxes(self, outputs, gt_instances: List[Instances],
        ...
        losses['avg_factor_bbox'] = avg_factor
        return losses
    ...
    def loss_labels(self,
        ...
        losses = {"loss_cls": loss_ce, "avg_factor_cls": avg_factor}
    ...
    def loss_mem_bank(self,
        ...
        losses = {"ref_loss_cls": loss_cls, "ref_avg_factor_cls": cls_avg_factor}
        ...
        matched_bbox_weights = torch.zeros((len(track_instances),len(self.code_weights)), dtype=torch.float32, device=device)
        for track_idx, id in enumerate(track_instances.obj_idxes):
            cpu_id = id.cpu().numpy().tolist()
            if cpu_id not in obj_idx_to_gt_idx.keys():
        ...
        bbox_weights = matched_bbox_weights * torch.tensor(self.code_weights).to(device)
        ...
        losses['ref_loss_box'] = loss_bbox
        losses['ref_avg_factor_box'] = pos_bbox_num
    ...
    def loss_prediction(self,
        ...
        loss_dict = {
            loss_key+'_loss': loss_prediction,
            loss_key+'_avg_factor': 1.0,
        }
        return loss_dict